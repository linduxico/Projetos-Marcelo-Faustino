...
public class Bloco extends JFrame implements ActionListener {
	...
	private int[][] winCombinations = { { 1, 2, 3 }, { 4, 5, 6 }, { 7, 8, 9 }, { 1, 4, 7 }, { 2, 5, 8 }, { 3, 6, 9 },
			{ 1, 5, 9 }, { 3, 5, 7 } };
...
// jPanelTabuleiro.setPreferredSize(new Dimension(500, 500));
			...
			jPanelTabuleiro.setBorder(BorderFactory.createTitledBorder(
					...
					TitledBorder.DEFAULT_POSITION, new Font("Arial", Font.BOLD, 18), new Color(30, 144, 255)));
		}
	...
	private JButton getJlbLogo() {
		if (jlbLogo == null) {
			ImageIcon imagen = new ImageIcon(getClass().getResource("/imagens/MALBATAHAN.png"));
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					JOptionPane.showMessageDialog(null,
							"Este jogo foi implementado com o algoritmo minimax, ou seja, usando inteligência artificial. "
									...
									+ "\n\n - Wikipédia",
							"Jogo Da Velha - J. Marcos B.", JOptionPane.INFORMATION_MESSAGE);
				}
			});
		}
		return jlbLogo;
...
// Deixa o programa no meio da tela, centralizado
	...
	public int table_winner(int[] cc) {
		...
		for (int i = 0; i <= 7; i++) {
			if ((cc[winCombinations[i][0]] == 1) && (cc[winCombinations[i][0]] == cc[winCombinations[i][1]])
					&& (cc[winCombinations[i][1]] == cc[winCombinations[i][2]]) && (cc[winCombinations[i][0]] != 0)) {
				game_over = true;
				winn = "X";
				rez = -1000000;
			}

			if ((cc[winCombinations[i][0]] != 2) || (cc[winCombinations[i][0]] != cc[winCombinations[i][1]])
					|| (cc[winCombinations[i][1]] != cc[winCombinations[i][2]]) || (cc[winCombinations[i][0]] == 0)) {
				continue;
			}
			game_over = true;
	...
	public void display_winner(int vinn, int infrang, int egal, String tex) {
		if (JOptionPane.showConfirmDialog(null,
				vinn + " Vitórias  ," + egal + "  Empates ," + infrang + "  Derrotas\n" + "Jogar de novo?", tex,
				0) != 0) {
...
// Metodo Checar vencedor
	public void checkWin() {
		...
		for (int c = 1; c <= 9; c++) {
			if ((buttons[c].getText().equals("X")) || (buttons[c].getText().equals("O"))) {
				nr++;
			}
		}
		if (wins == true) {
